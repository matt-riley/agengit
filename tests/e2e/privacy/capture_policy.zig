...
test "privacy/capture_policy malformed config fails closed during capture" {
    var sandbox = try harness.Sandbox.init(std.testing.allocator);
    defer sandbox.deinit();

    try sandbox.writeRepoFile(".agit/config.json", "{not-json");

    const user_payload = try std.fmt.allocPrint(std.testing.allocator,
        \\{{"session_id":"failclosed-sess","transcript_path":"transcript.jsonl","cwd":"{s}","hook_event_name":"UserPromptSubmit","prompt":"debug auth"}}
    , .{sandbox.cwd});
    defer std.testing.allocator.free(user_payload);
    const tool_payload = try std.fmt.allocPrint(std.testing.allocator,
        \\{{"session_id":"failclosed-sess","transcript_path":"transcript.jsonl","cwd":"{s}","hook_event_name":"PostToolBatch","tool_calls":[{{"tool_name":"Bash","tool_input":{{"command":"printenv TOKEN"}},"tool_use_id":"toolu_01","tool_response":"secret-token-value"}}]}}
    , .{sandbox.cwd});
    defer std.testing.allocator.free(tool_payload);
    const stop_payload = try std.fmt.allocPrint(std.testing.allocator,
        \\{{"session_id":"failclosed-sess","transcript_path":"transcript.jsonl","cwd":"{s}","hook_event_name":"Stop","stop_hook_active":false,"last_assistant_message":"done"}}
    , .{sandbox.cwd});
    defer std.testing.allocator.free(stop_payload);
    ...
    const object_path = try std.fmt.allocPrint(std.testing.allocator, ".agit/objects/{s}/{s}", .{
        step_hash[0..2],
        step_hash[2..],
    });
    defer std.testing.allocator.free(object_path);
    const raw_object = try sandbox.readRepoFileAlloc(object_path);
    defer std.testing.allocator.free(raw_object);
    try std.testing.expect(std.mem.indexOf(u8, raw_object, "secret-token-value") == null);
    try std.testing.expect(std.mem.indexOf(u8, raw_object, "capture disabled") != null);
}
