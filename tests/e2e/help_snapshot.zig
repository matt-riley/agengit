...
test "golden/help/grep" {
    ...
    var result = try sandbox.run(&.{ "grep", "--help" }, null);
    defer result.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0), result.exit_code);

    try golden.assertGolden(&sandbox, "tests/golden/help/grep.txt", result.stdout);
}

test "golden/help/recall" {
    var sandbox = try harness.Sandbox.init(std.testing.allocator);
    defer sandbox.deinit();

    var result = try sandbox.run(&.{ "recall", "--help" }, null);
    defer result.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0), result.exit_code);

    try golden.assertGolden(&sandbox, "tests/golden/help/recall.txt", result.stdout);
}
...
test "golden/help/stats" {
    var sandbox = try harness.Sandbox.init(std.testing.allocator);
    defer sandbox.deinit();

    var result = try sandbox.run(&.{ "stats", "--help" }, null);
    defer result.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0), result.exit_code);

    try golden.assertGolden(&sandbox, "tests/golden/help/stats.txt", result.stdout);
}
...
test "golden/help/privacy" {
    var sandbox = try harness.Sandbox.init(std.testing.allocator);
    defer sandbox.deinit();

    var result = try sandbox.run(&.{ "privacy", "--help" }, null);
    defer result.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0), result.exit_code);

    try golden.assertGolden(&sandbox, "tests/golden/help/privacy.txt", result.stdout);
}
...
test "golden/help/fsck" {
    var sandbox = try harness.Sandbox.init(std.testing.allocator);
    defer sandbox.deinit();

    var result = try sandbox.run(&.{ "fsck", "--help" }, null);
    defer result.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0), result.exit_code);
