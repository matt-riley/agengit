...
test "init/fresh" {
    ...
    try expectContains(result.stdout, "wrote Copilot CLI extension");
    try expectContains(result.stdout, "wrote Pi extension");
    try expectDoesNotContain(result.stdout, "points to a different agit binary");
    ...
    const copilot = try readHomeConfig(&sandbox, ".copilot/extensions/agit-recorder/extension.mjs");
    ...
    try expectContains(copilot, "Generated by `agit init`");
    try expectContains(copilot, "\"copilot-hook\"");
    try expectContains(copilot, "onUserPromptSubmitted");
    try expectContains(copilot, "onPostToolUse");
    try expectContains(copilot, "onAgentStop");

    const pi_ext = try readHomeConfig(&sandbox, ".pi/agent/extensions/agit-recorder.js");
    defer std.testing.allocator.free(pi_ext);
    try expectContains(pi_ext, "Generated by `agit init`");
    try expectContains(pi_ext, "\"pi-hook\"");
    try expectContains(pi_ext, "before_agent_start");
    try expectContains(pi_ext, "tool_execution_end");
    try expectContains(pi_ext, "agent_end");
    try expectContains(pi_ext, sandbox.agit_bin);
}
