...
test "init/force" {
    ...
    var result = try sandbox.run(&.{ "init", "--force" }, null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, config, "\"hooks\"") != null);

    const bak = try readHome(&sandbox, ".claude/settings.json.agit.bak");
    defer std.testing.allocator.free(bak);
    try std.testing.expectEqualStrings("{not-json", bak);
}
