...
test "installJsExtension warns on stale binary path" {
    ...
    try tmp.dir.createDirPath(io, ".pi/agent/extensions");
    var root_path: [std.fs.max_path_bytes]u8 = undefined;
    const root_len = try tmp.dir.realPath(io, &root_path);

    const config_path = try std.fmt.allocPrint(gpa, "{s}/.pi/agent/extensions/agit-recorder.js", .{
        root_path[0..root_len],
    });
    defer gpa.free(config_path);

    {
        var existing = try std.Io.Dir.cwd().createFile(io, config_path, .{ .truncate = true });
        defer existing.close(io);
        try existing.writeStreamingAll(io,
            \\// Generated by `agit init`
            \\const AGIT_BINARY = "/old/agit";
            \\
        );
    }

    const agent_plan = init_plan_mod.AgentPlan{
        .agent = init_plan_mod.pi,
        .state = .ready,
    ...
    {
        var stdout_file = try tmp.dir.createFile(io, "stdout.txt", .{ .truncate = true });
        defer stdout_file.close(io);
        var stdout_buf: [4096]u8 = undefined;
        var stdout = stdout_file.writer(io, &stdout_buf);

        try installJsExtension(io, gpa, agent_plan, "/new/agit", false, &stdout);
        try stdout.flush();
    }

    const stdout_text = try tmp.dir.readFileAlloc(io, "stdout.txt", gpa, .unlimited);
    defer gpa.free(stdout_text);
    try std.testing.expect(std.mem.indexOf(u8, stdout_text, "points to a different agit binary") != null);

    const installed = try std.Io.Dir.cwd().readFileAlloc(io, config_path, gpa, .unlimited);
    defer gpa.free(installed);
    try std.testing.expect(std.mem.indexOf(u8, installed, "/new/agit") != null);
}
