...
/// Remove a generated JS extension file (Pi). Only deletes the file when it was
/// written by agit (identified by the generated-file marker).
fn uninstallJsExtension(
    io: std.Io,
    gpa: std.mem.Allocator,
    home: []const u8,
    rel_path: []const u8,
    ...
    const path = try std.mem.concat(gpa, u8, &.{ home, "/", rel_path });
    defer gpa.free(path);

    const contents = std.Io.Dir.cwd().readFileAlloc(io, path, gpa, .unlimited) catch |err| switch (err) {
        error.FileNotFound => return,
        else => {
            try stdout.interface.print("agit uninstall: could not read {s}: {s}\n", .{ path, @errorName(err) });
            return;
        },
    };
    defer gpa.free(contents);

    if (std.mem.indexOf(u8, contents, js_extension_marker) == null) {
        try stdout.interface.print("agit uninstall: leaving non-agit extension {s}\n", .{path});
        return;
    }

    std.Io.Dir.cwd().deleteFile(io, path) catch |err| {
        try stdout.interface.print("agit uninstall: failed to remove {s}: {s}\n", .{ path, @errorName(err) });
        return;
    };
    try stdout.interface.print("agit uninstall: removed agit extension {s}\n", .{path});
}
