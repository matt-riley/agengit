...
pub fn run(
    ...
    try checkConfigTmpFiles(io, gpa, home, "copilot", ".copilot/extensions/agit-recorder/extension.mjs", &stdout);
    ...
    if (!try checkJsExtensionAgent(io, gpa, environ, home, exe, "copilot", ".copilot/extensions/agit-recorder/extension.mjs", &stdout)) has_failures = true;
    if (!try checkJsExtensionAgent(io, gpa, environ, home, exe, "pi", ".pi/agent/extensions/agit-recorder.js", &stdout)) has_failures = true;
...
fn runJson(
    ...
    try appendConfigTmpCheck(io, gpa, aa, home, "copilot", ".copilot/extensions/agit-recorder/extension.mjs", &checks);
    ...
    try checks.append(gpa, try collectJsExtensionCheck(io, gpa, environ, aa, home, exe, "copilot", ".copilot/extensions/agit-recorder/extension.mjs"));
    try checks.append(gpa, try collectJsExtensionCheck(io, gpa, environ, aa, home, exe, "pi", ".pi/agent/extensions/agit-recorder.js"));
...
/// Doctor check for a JS-extension agent (for example Copilot or Pi): the
/// extension is a generated file rather than merged JSON config, so confirm the
/// file exists, was written by agit, and embeds the current agit binary path.
fn collectJsExtensionCheck(
    io: std.Io,
    gpa: std.mem.Allocator,
    environ: std.process.Environ,
    aa: std.mem.Allocator,
    ...
    const text = (readFileAllocOrNull(io, aa, config_path) catch |err| {
        return .{
            .code = "agent_config_unreadable",
            .status = .@"error",
            .message = try std.fmt.allocPrint(aa, "{s} extension is unreadable.", .{agent_name}),
            .hint = @errorName(err),
            .path = try aa.dupe(u8, config_path),
        };
    }) orelse {
        return .{
            .code = "agent_config_missing",
            .status = .@"error",
            .message = try std.fmt.allocPrint(aa, "{s} extension file not found.", .{agent_name}),
            .path = try aa.dupe(u8, config_path),
        };
    };

    if (std.mem.indexOf(u8, text, js_extension_marker) == null) {
        return .{
            .code = "agent_not_initialized",
            .status = .@"error",
            .message = try std.fmt.allocPrint(aa, "{s} extension is not an agit-generated file.", .{agent_name}),
            .path = try aa.dupe(u8, config_path),
        };
    }

    if (std.mem.indexOf(u8, text, exe) != null) {
        return .{
            .code = "agent_hooks_configured",
            .status = .ok,
            .message = try std.fmt.allocPrint(aa, "{s} extension configured for the current agit binary.", .{agent_name}),
            .path = try aa.dupe(u8, config_path),
        };
    }
    ...
    return .{
        .code = "agent_binary_mismatch",
        .status = .@"error",
        .message = try std.fmt.allocPrint(aa, "{s} extension points at a different agit binary.", .{agent_name}),
        .hint = try std.fmt.allocPrint(aa, "current={s}", .{exe}),
        .path = try aa.dupe(u8, config_path),
    };
}
...
fn checkJsExtensionAgent(
    ...
    const text = (readFileAllocOrNull(io, aa, config_path) catch |err| {
        try stdout.interface.print("  ✗ {s}: extension unreadable ({s}): {s}\n", .{ agent_name, config_path, @errorName(err) });
        return false;
    }) orelse {
        try stdout.interface.print("  ✗ {s}: extension not found ({s})\n", .{ agent_name, config_path });
        return false;
    };

    if (std.mem.indexOf(u8, text, js_extension_marker) == null) {
        try stdout.interface.print("  ✗ {s}: agit not initialized (not an agit-generated extension)\n", .{agent_name});
        return false;
    }

    if (std.mem.indexOf(u8, text, exe) != null) {
        try stdout.interface.print("  ✓ {s}: extension configured (binary matches)\n", .{agent_name});
        return true;
    }

    try stdout.interface.print(
        "  ✗ {s}: binary mismatch (extension does not reference current {s})\n",
        .{ agent_name, exe },
    );
    return false;
}
